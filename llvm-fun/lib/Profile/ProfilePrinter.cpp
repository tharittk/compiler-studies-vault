...
static cl::opt<bool>
toDumpBBInfo("dump-bb-info", cl::init(false), cl::NotHidden, cl::Optional,
             cl::desc("Print BB profiling info"));

static cl::opt<bool>
toDumpEdgeInfo("dump-edge-info", cl::init(false), cl::NotHidden, cl::Optional,
    ...
    for (auto &kv: name2BBMap) {
      ...
      for (auto &kv : name2PredMap) {
        ...
        fout << bb->getName().str() << " -> " << succ->getName().str()
             << ": " << loader.getCount(edge) << " ("
             << loader.getWeight(edge) * 100 << "%)\n";
      }
    }
    fout << "\n";
  }
